...
for channelId=1:length(channelsToProcess)
    ...
    fname=[basedirData  '/' prefix num2str(channel) '_cells' '.mat'];
    if exist(fname)~=2
        continue;
    end
    ...
    for i=1:length(cells)
        ...
        params.meanWaveform_recog = meanWaveform_recog(find([meanWaveform_recog.clust] == origClusterIDs )).m_waveform;
        params.meanWaveform_learn = meanWaveform_learn(find([meanWaveform_recog.clust] == origClusterIDs )).m_waveform;
