...
%
%===
function [FPOld,TPOld, FPNew,TPNew, typeCounters, percError, percCorrect,errorRateLearn, RTsRecog, RTsLearn,percAccuracy_high, percAccuracy_low, responses, recogState,RTsRecog_vsQonset,StimOff_toQ_delay,StimOn_toOff_delay ] = NObehaviorROC(NOsession, NOsessionLearn, doPlot, basepath, modeExcludeSlowRT)
if nargin<4
    basepath='/data/';
end
if nargin<5
...
[tmp, stimuliRecog, newOldRecogLabels, responses, responsesEventfile,tmp,tmp,tmp,tmp,tmp,tmp,RTsRecog, RTsRecog_vsQonset,StimOff_toQ_delay,StimOn_toOff_delay] = NOloadDataOfBlock_release( NOsession, extractionMode, basepath);
extractionMode=2; %learn
if NOsession.EXPERIMENTIDLearn>-1
     [stimuliLearn, tmp, tmp, tmp, responsesLearn,tmp,tmp,tmp,tmp,tmp,errorRateLearn,RTsLearn] = NOloadDataOfBlock_release( NOsession, extractionMode, basepath);
...
if length(stimuliLearn)==0
    if ~isempty(NOsessionLearn)
        [stimuliLearn, tmp, tmp, tmp, tmp,tmp,tmp,tmp,tmp,tmp,errorRateLearn,RTsLearn] = NOloadDataOfBlock_release( NOsessionLearn, extractionMode,basepath);
    else
        RTsLearn=nan;
        errorRateLearn=nan;
        stimuliLearn=nan;
